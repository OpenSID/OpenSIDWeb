...
class MY_Model extends CI_Model
{
    ...
    public function relate($row)
    {
        ...
        foreach ($this->belongs_to as $key => $value)
        {
            ...
            {
                $alias_model = $relationship . '_model';
                if(isset($options['alias'])){
                  $alias_model = $options['alias'];
                }

                $this->load->model($options['model'],$alias_model);

                if (is_object($row))
                {
                    $row->{$relationship} = $this->{$alias_model}->get($row->{$options['primary_key']});
                }
                else
                {
                    $row[$relationship] = $this->{$alias_model}->get($row[$options['primary_key']]);
        ...
        foreach ($this->has_many as $key => $value)
        {
            ...
            if (in_array($relationship, $this->_with))
            {
                ...
                $this->load->model($options['model'],$alias_model);
                /* we can add parameter for searching on has_many relationship */
                $add_parameter = isset($options['where']) ? (is_array($options['where']) ? $options['where'] : array() ) : array();
                $search_parameter = $add_parameter;
                if (is_object($row))
                {
                    $search_parameter[$options['primary_key']] = $row->{$this->primary_key};
                    $row->{$relationship} = $this->{$alias_model}->get_many_by($search_parameter);
                }
                else
                {
                    $search_parameter[$options['primary_key']] = $row[$this->primary_key];
    ...
    /* tambahan saya supaya bisa ambil fields tertentu saja*/
    /**
